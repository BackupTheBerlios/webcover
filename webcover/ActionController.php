<?php
    $_start = microtime();
  //
  // $Id: ActionController.php,v 1.1 2004/04/27 16:25:28 jjhop Exp $
  // Author: Rafa³ Kotusiewicz <jjhop@codeguru.info>
  // 
  // This file is part of the WebCover Framework
  // http://codeguru.info/_projects/webcover
  //
  // This application is free software; you can redistribute it and/or
  // modify it under the terms of the Ruby license defined in the
  // LICENSE file.
  // 
  // Copyright (c) 2004 Rafa³ Kotusiewicz. All rights reserved.
  //
  
  function log_($t) {
          print "<font color=\"red\">$t</font><br>";
  }
	//ini_set('session.name', 'ITVPSESSID');
	session_start();
	// DEBUG: print ("<a href=\"" . $_SERVER['SCRIPT_URL'] ."\">" . $_SERVER['REQUEST_URI'] . "</a><br>");
    ...
    */
    ...
    $last_action_path;      // ¶cia¿ka do ostatniej akcji
	...
	class ActionControler {
		...
		*/
		function parse_config() {
			...
			if(!function_exists('xml_parser_create_ns')) {
				// brak obs³ugi XML'a :(
				die('brak obs³ugi XMLa w serwerze');
			} else {
                ...
                */                
                function start_tag($parser, $tag_name, $attribs) {
                    ...
                    //log_($current_tag);   
                    switch($current_tag) {
                        case 'http://codeguru.info/_projects/webcover:smarty-config':
                            $smarty_config_tag = true;
                            break;
                        case 'http://codeguru.info/_projects/webcover:actions-conf':
                            $actions_conf_tag = true;
                            break;
                        case 'http://codeguru.info/_projects/webcover:action-paths':
                            $action_paths_tag = true;
                            break;
                        case 'http://codeguru.info/_projects/webcover:template-paths':
                            $template_paths_tag = true;
                            break;
                        case 'http://codeguru.info/_projects/webcover:global-forwards':
                            $global_forwards_tag = true;
                            break;
                        case 'http://codeguru.info/_projects/webcover:actions-mapping':
                            $actions_mapping_tag = true;
                            break;
                        case 'http://codeguru.info/_projects/webcover:action':
                            $action_tag = true;
                        default:
                            break;
                    }
                    
                    if($tag_name == 'http://codeguru.info/_projects/dbxml:db-conf') {
                    ...
                    */
                    if($tag_name == 'http://codeguru.info/_projects/webcover:cache-dir') {
                    ...
                    */
                    ...
                    if($tag_name == 'http://codeguru.info/_projects/webcover:forward' AND $global_forwards_tag == true) {
                        //log_($attribs['name']);
                        $name_ = $attribs['name'];
                        $type_ = $attribs['type'];
                        $path_ = $attribs['path'];
                    ...
                    */
                    if($tag_name == 'http://codeguru.info/_projects/webcover:action' AND $actions_mapping_tag == true) {
                        // mamy jak±¶ akcjê
                    ...
                    */
                    if($tag_name == 'http://codeguru.info/_projects/webcover:forward' AND $actions_mapping_tag == true) {
                ...
                function characters_data($parser, $data) {
                    ...
                    */
                    if($current_tag == 'http://codeguru.info/_projects/dbxml:type-connect') {
                        $configuration['db_conf']['type_connect']	= $data;
                    } elseif($current_tag == 'http://codeguru.info/_projects/dbxml:host') {
                        $configuration['db_conf']['host']			= $data;
                    } elseif($current_tag == 'http://codeguru.info/_projects/dbxml:port') {
                        $p = (int)$data;
                        if($p != $data)  
                            die(htmlspecialchars('<db-conf><port>' . $data . '</port></db-conf> must be integer!'));
                        else 
                            $configuration['db_conf']['port']		= $p;
                    } elseif($current_tag == 'http://codeguru.info/_projects/dbxml:user') {
                        $configuration['db_conf']['user']			= $data;
                    } elseif($current_tag == 'http://codeguru.info/_projects/dbxml:pass') {
                        $configuration['db_conf']['pass']			= $data;
                    } elseif($current_tag == 'http://codeguru.info/_projects/dbxml:dbname') {
                    ...
                    */
                    if($current_tag == 'http://codeguru.info/_projects/webcover:smarty-dir') {
                        $configuration['smarty_config']['smarty_dir']  = $data;
                    } elseif($current_tag == 'http://codeguru.info/_projects/webcover:compile-dir') {
                        $configuration['smarty_config']['compile_dir'] = $data;
                    } elseif($current_tag == 'http://codeguru.info/_projects/webcover:config-dir') {
                        $configuration['smarty_config']['config_dir']  = $data;
                    } elseif($current_tag == 'http://codeguru.info/_projects/webcover:cache-dir') {
                    ...
                    */
                    if($current_tag == 'http://codeguru.info/_projects/webcover:action-suffix') {
                        $configuration['actions_conf']['action_suffix'] = $data;
                    } elseif($current_tag == 'http://codeguru.info/_projects/webcover:template-suffix') {
                    ...
                    */
                    if($current_tag == 'http://codeguru.info/_projects/webcover:path' AND $action_paths_tag == true)
                    {
                        $configuration['actions_conf']['action_paths'][] = $data;
                    }
                    elseif($current_tag == 'http://codeguru.info/_projects/webcover:path' AND $template_paths_tag == true) 
                    {
        ...
        */
		function execute() {
            ...
            //ini_set("include_path", ini_get("include_path") . ":/test");
